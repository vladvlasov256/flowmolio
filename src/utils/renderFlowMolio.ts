 ...
 */
function applyColorNodes(svgTree: any, nodes: NodeData[], connections: Connection[], dataSources: any): void {
  ...
  colorNodeConnections.forEach(connection => {
    ...
    const colorNode = nodes.find(n => 
      n.id === connection.targetNodeId && n.type === 'color'
    ) as ColorNodeData | undefined;
    ...
    const applyColorToElements = (element: any) => {
      ...
      if (colorNode.enabledRoles[ColorRole.STOP_COLOR] && 
 ...
 */
export function renderFlowMolio(
  ...
  try {
    ...
    if (previewObject.nodes && previewObject.nodes.length > 0 && 
        previewObject.connections && previewObject.connections.length > 0) {
